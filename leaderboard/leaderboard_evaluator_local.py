...
class LeaderboardEvaluator(object):
    ...
    def __init__(self, args, statistics_manager, environment, evaluator):
        """
        ...
        self.client = (
            self.environment.get_client()
        )  # carla.Client(args.host, int(args.port))
        if args.timeout:
    ...
    def _load_and_wait_for_world(self, args, town):
        ...
        CarlaDataProvider.set_traffic_manager_port(int(args.tm_port))

        self.traffic_manager.set_synchronous_mode(True)
        self.traffic_manager.set_random_device_seed(int(args.tm_seed))
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            self.agent_instance = getattr(self.module_agent, agent_class_name)(
                args.agent_config, args.device
            )
            self.agent_instance.set_environment(self.environment)
            self.agent_instance.set_evaluator(self.evaluator)
        ...
        # self.agent_instance.configure_bev_module()
...
def main(config, device, environment, evaluator):
    arguments = Namespace(**config)
    ...
    try:
        leaderboard_evaluator = LeaderboardEvaluator(
            arguments, statistics_manager, environment, evaluator
        )
